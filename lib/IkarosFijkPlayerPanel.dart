...
FijkPanelWidgetBuilder ikarosFijkPanelBuilder(
    {Key? key,
...
class IkarosFijkPlayerPanel extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _IkarosFijkPlayerPanelState();
  }
}
...
class _IkarosFijkPlayerPanelState extends State<IkarosFijkPlayerPanel> {
  ...
  void onVerticalDragStartFun(DragStartDetails d) {
    if (d.localPosition.dx > panelWidth() / 2) {
      ...
      FijkVolume.getVol().then((v) {
        if (!widget.data.contains(_fijkViewPanelVolume)) {
          widget.data.setValue(_fijkViewPanelVolume, v);
        }
        setState(() {
          _volume = v;
      ...
      FijkPlugin.screenBrightness().then((v) {
        if (!widget.data.contains(_fijkViewPanelBrightness)) {
          widget.data.setValue(_fijkViewPanelBrightness, v);
        }
        setState(() {
          _brightness = v;
          _valController.add(v);
